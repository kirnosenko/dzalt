...
namespace DZALT.Tests.Entities.Tracing
{
	public class LineTracerTest : BaseRepositoryTest
	{
		...
		[Theory]
		[InlineData(1, EventLog.EventType.MURDER)]
		[InlineData(5, EventLog.EventType.MURDER)]
		[InlineData(6, EventLog.EventType.ACCIDENT)]
		public async Task ShouldReconsiderAnAccidentAsAMurderAfterHitByPlayer(int minutes, EventLog.EventType @event)
		{
			...
			player.Should().NotBeNull();
			enemy.Should().NotBeNull();
			log2.PlayerId.Should().Be(player.Id);
			log2.Date.Should().Be(new DateTime(1, 1, 1, 10, 11, 12));
			log2.X.Should().Be(11492.3M);
			log2.Y.Should().Be(5503.6M);
			log2.Z.Should().Be(243.2M);
			log2.Event.Should().Be(@event);
			log2.EnemyPlayerId.Should().Be(@event == EventLog.EventType.MURDER ? enemy.Id : null);
			log2.EnemyPlayerX.Should().BeNull();
			log2.EnemyPlayerY.Should().BeNull();
			log2.EnemyPlayerZ.Should().BeNull();
			log2.Damage.Should().BeNull();
			log2.Health.Should().BeNull();
			log2.Enemy.Should().BeNull();
			log2.BodyPart.Should().BeNull();
			log2.Hitter.Should().BeNull();
			log2.Weapon.Should().BeNull();
			log2.Distance.Should().BeNull();
		}
